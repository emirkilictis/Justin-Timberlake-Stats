...
- **`extra_track_sales_us`** (opsiyonel, albüm): vault'ta satırı **olmayan** albüm şarkılarının ABD download toplamı. RIAA albüm TEA'sı albümdeki *bütün* şarkıları sayar (10 şarkı = 1 albüm); vault yalnızca single'ları tuttuğu için albümün geri kalanı TEA'ya hiç girmiyordu. `value` (ABD adet) + `worldwide_downloads` (şarkı şarkı, ChartMasters) + `us_share` + `evidence` + `source`; `evidence`/`source` yoksa `resolveExtraTrackSales` bloğu yok sayar. Dünya → ABD çevrimi her albümün **kendi** ABD/dünya pure oranıyla yapılıyor (bizde zaten yazılı iki rakam), yeni varsayım yok. ChartMasters'ın satış girdileri Şubat 2018'den kalma; 2018 sonrası download'lar dahil değil. **Bir şarkı vault'a eklenirse bu listeden çıkarılmalı**, yoksa TEA'da iki kez sayılır.
- **Albüm SEA'sında ölçülmüş pay:** albüm tek bir dönem payı kullanır; ama içindeki bir şarkının `us_share`'i varsa (Suit & Tie) o şarkının stream'leri albüm SEA'sına ölçülmüş payla girer — yalnızca fark eklenir. Video, şarkının klibi albümün sertifika video listesindeyse düzeltilir.
- **Albüm bazlı video listesi — `albumCertVideoIds`:** albüm SEA'sı `data.json`'daki albüm listesi + vault albümünün kendi `youtubeVideoIds`'i ile hesaplanır, **başka bir vault albümünün sahiplendiği videolar düşülür**. 2 of 2'nin videoları (TKO, Not a Bad Thing, Take Back the Night) vault'ta 2 of 2 albümüne yazılı; 2026-09'a kadar part 1'in SEA'sına sayılıyordu. Holy Grail klibi de 20/20 listesindeydi (hem 20/20'ye hem Orphan'a sayılıyordu), çıkarıldı.
- **İki 20/20 cildi, tek video havuzu:** `data.json` iki cildi tek kayıtta tutuyor. Kendi YT ID'si olmayan şarkının video payı `albumVideoPool` ile **iki cildin birleşik Spotify toplamına** oranlanıyor. Eskiden yalnızca part 1'e bölünüyordu.
- **Take Back the Night / Drink You Away** 2 of 2 şarkıları; vault'ta 2026-09'a kadar part 1'e yazılıydı ve TEA'ları yanlış albüme gidiyordu. (song-map zaten doğruydu, stream'leri doğru yerdeydi.)
...
- **song-map anahtarları vault'ta `normalizeKworbTitle`'dan geçirilerek eşleştiriliyor.** Başlık `&`→`and` çevrildiği hâlde anahtar ham kalınca "Suit & Tie"ın 380M stream'i 2026-09'a kadar hiçbir albüme bağlanmıyordu. (Ana sayfa ve streams normalize etmeden eşleştirdiği için orada sorun yoktu.)
...
- **"4 Minutes" altı sürüm, JT'nin Kworb sayfasında ikisi var.** Diğer dördü (112M'lik ikinci yayın + Live/Peter Saves/Junkie XL) yalnızca Madonna'nın sayfasında. `four-minutes.js` bunları `api/kworb.js` üzerinden çekip ekliyor; vault, streams, album ve ana sayfa (`script.js`) aynı fonksiyonu çağırıyor. Ana sayfa 2026-09'a kadar kendi `is4MinTrack` kontrolünü kullanıyordu; JT'nin kendi 550M'lik satırını görüp eksik sürümleri hiç eklemiyordu ve Spotify toplamı streams sayfasının tam 121M altında kalıyordu. **Yeni bir sayfa 4 Minutes sayacaksa kendi kontrolünü yazmasın, bu fonksiyonu çağırsın.** Hangi satırın eksik olduğu başlık yazımından (`&` / `and`) **değil**, JT'nin canlı listesiyle farktan belirleniyor — Kworb bir sürümü JT sayfasına eklerse kendiliğinden elenir. `normalizeKworbTitle` `&`→`and` çevirdiği için bu dedupe'ta **kullanılamaz**; `fourMinKey` kullanılır.
